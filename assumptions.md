...
| A17 | Self-hosted GPU: g5.xlarge (1× A10G, 24 GB VRAM) at ~$1.006/hr on-demand, ~$0.523/hr 1-year reserved, ~$0.35/hr spot. vLLM with INT8/FP8 quantization achieves ≥825 tokens/sec on Llama 3.1 8B at 80% GPU utilization on A10G. EKS cluster + reserved baseline fixed cost: ~$1,453/month. | The break-even analysis, the 10K docs/day adoption threshold, and the cost comparison table in Section 14.3. | If spot pricing rises or vLLM throughput falls below 825 tokens/sec (e.g., due to long-context performance degradation), the break-even volume increases. Benchmark with production-representative 18K-token prompts before switching. |
...
## Stage 7 — Storage, Retrieval, and Notification

| # | Assumption | Justifies | Impact if Wrong |
|---|---|---|---|
| A21 | Stage 7 cost: output storage/write ≤`$0.001/100 pages`; retrieval index + webhook dispatch ≤`$0.001/job`; hard stop-loss at `$0.005/job`. Storage cost accumulates over the 365-day retention lifetime and must be tracked separately from per-job processing cost. | The claim that Stage 7 leaves sufficient headroom within the `$0.10/100-page` pipeline target and that retained-storage cost is visible in dashboards. | If output artifacts are large (many fields, tables, citations) or retention periods are extended, per-job storage cost grows. Cost dashboards must distinguish processing cost from retention cost. |
| A22 | Webhook delivery: 8 attempts maximum, 24-hour max window, exponential backoff with jitter, 5-second timeout per attempt. First delivery attempt within 5 seconds of terminal state. | The webhook reliability claim and the operational metric for time-to-first-notification. | If customer endpoints are frequently unavailable for more than 24 hours, webhooks will exhaust retries and require operator-initiated replay. Tenant-specific delivery windows are a v2 concern. |
| A23 | Signed URL TTL: 15 minutes default. This is sufficient for a customer to receive the webhook, call the retrieval API, and download the result. | The retrieval API design and the claim that signed URLs do not require persistent pre-signing infrastructure. | If customers operate batch pipelines that call the retrieval API after a significant delay, the URL may expire. The customer must call the retrieval API again to get a fresh URL — this is by design. |
